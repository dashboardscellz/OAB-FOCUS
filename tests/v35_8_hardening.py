...
def test_all_data_scripts_v358():
    h=INDEX.read_text(encoding='utf-8');tags=re.findall(r'<script[^>]*src="(data/[^"]+\.js(?:\?[^\"]*)?)"[^>]*>',h);assert len(tags)>=25;assert all('?v=' in t for t in tags),[t for t in tags if '?v=' not in t]

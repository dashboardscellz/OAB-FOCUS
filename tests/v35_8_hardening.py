...
def test_state_service_and_context_migration():
    h=INDEX.read_text(encoding='utf-8');assert STATE.exists();assert 'src="data/v35-state.js?v=35.9"' in h;assert h.index('data/v35-state.js?v=35.9')<h.index('data/v15-patch.js?v=35.9')
...
def test_all_data_scripts_v358():
    h=INDEX.read_text(encoding='utf-8');tags=re.findall(r'<script[^>]*src="(data/[^"]+\.js(?:\?[^\"]*)?)"[^>]*>',h);assert len(tags)>=25;assert all('?v=35.9' in t for t in tags),[t for t in tags if '?v=35.9' not in t]

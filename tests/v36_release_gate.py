...
def test_v36_assets_load_after_v35_assets():
    for f in ['v36-content-governance.js','v36-curated-questions.js','v36-primary-prep.js']:
        assert f'data/{f}?v=36.2' in INDEX
    assert 'data/v36-primary-prep.css?v=36.2' in INDEX
    assert INDEX.index('data/v35-taxonomy.js?v=36.2') < INDEX.index('data/v36-primary-prep.js?v=36.2')

def test_all_local_data_scripts_use_v36_cache_key():
    tags=re.findall(r'<script[^>]*src="(data/[^"]+\.js(?:\?[^\"]*)?)"[^>]*>',INDEX)
    assert tags and all('?v=36.2' in t for t in tags),[t for t in tags if '?v=36.2' not in t]

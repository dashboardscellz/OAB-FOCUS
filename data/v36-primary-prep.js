(function(root){
  ...
  function injectPreparePanel(){
    ...
    panel.innerHTML=`<div class="v36-readiness-head"><div><span class="eyebrow">DIREÇÃO DE ESTUDO</span><h3>Prontidão de estudo: ${snap.readiness}/100</h3><p>${snap.note}</p></div><div class="v36-readiness-score"><strong>${snap.coverage}%</strong><small>cobertura ponderada</small><strong>${snap.accuracy}%</strong><small>aproveitamento observado</small></div></div><div class="v36-mission-grid">${prior.map((x,i)=>`<article><span>${i+1}</span><div><b>${escapeHtml(x.discipline)} · ${escapeHtml(x.topic||x.target?.title||'prioridade')}</b><p class="v36-priority-reason">${escapeHtml(x.v36Reason||'Prioridade calculada por incidência, domínio, revisão e cobertura.')}</p></div></article>`).join('')||'<article><span>1</span><div><b>Faça o diagnóstico inicial</b><p class="v36-priority-reason">Responda ao diagnóstico para calibrar prioridades, fraquezas e cobertura.</p></div></article>'}</div><div class="v36-source-row"><span class="v36-source-badge">FGV/OAB</span><span class="v36-source-badge">peso da prova</span><span class="v36-source-badge">erros</span><span class="v36-source-badge">revisão</span><span class="v36-source-badge">cobertura</span></div>${snap.answered<80?'<div class="v36-diagnostic-action"><button class="btn primary small" id="v36Diagnostic">Fazer diagnóstico de 40 questões</button><small>Use no início para calibrar a ordem do estudo. Questões oficiais têm prioridade.</small></div>':''}`;
